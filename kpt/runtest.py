...
def runTest(testCaseFile):
    ...
    randomPhone=str(random.randint(10000000, 999999999999999))
    randomIDNum=str(random.randint(100000, 999999)) + str(random.randint(10,30))+str(random.randint(10,12))
    randomDeviceID=''.join(random.sample('zyxwvutsrqponmlkjihgfedcba0123456789',30))
    for i in range(1, table.nrows):
        correlationDict['${randomPhone}'] = randomPhone
        correlationDict['${randomIDNum}'] = randomIDNum
        correlationDict['${randomDeviceID}'] = randomDeviceID
...
def interfaceTest(num, api_purpose, api_host, request_url, request_data, check_point, request_method, request_data_type,
    ...
    if session is not None:
        headers['Cookie'] = 'X-Sso-Token=' + session
    ...
    if request_method == 'POST':
        ...
        if len(request_data)==0:
            ...
            print(request_url + '?' + str(request_data))
...
def sendMail(text):
    ...
    mailToCc = ['815653824@qq.com']
    subject = '[AutomantionTest]接口自动化测试报告通知'
    smtpserver = 'smtp.qq.com'
    username = '815653824@qq.com'
...
def main():
    errorTest = runTest('TestCase.xlsx')
    if len(errorTest) > 0:
